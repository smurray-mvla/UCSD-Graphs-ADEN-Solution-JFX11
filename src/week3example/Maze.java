...
public class Maze {
	 ...
	 * @param col The column of a specfic cell
	 */
	...
	public void setPath(List<Coordinate> path) {
		...
		for (Coordinate n : path) {
			MazeNode node = cells[n.getRow()][n.getCol()];
			if (index == 0) {
	 ...
	 */
	public List<Coordinate> dfs(Coordinate begin, Coordinate end) {
		
		// Initialize everything
		MazeNode start = cells[begin.getRow()][begin.getCol()];
		MazeNode goal = cells[end.getRow()][end.getCol()];
	...
	private List<Coordinate> reconstructPath(MazeNode start, MazeNode goal, HashMap<MazeNode, MazeNode> parentMap) {
		// reconstruct the path
		LinkedList<Coordinate> path = new LinkedList<Coordinate>();
		MazeNode curr = goal;
		while (curr != start) {
			Coordinate coord = new Coordinate(curr.getRow(),curr.getColumn());
			path.addFirst(coord);
			curr = parentMap.get(curr);
		}
		Coordinate coord = new Coordinate(start.getRow(),start.getColumn());
		path.addFirst(coord);
		return path;
	}
	 ...
	 * an empty list if there is no path.
	 */
	public List<Coordinate> bfs(Coordinate begin, Coordinate end) {
		MazeNode start = cells[begin.getRow()][begin.getCol()];
		MazeNode goal = cells[end.getRow()][end.getCol()];
...
*/
	public static void main(String[] args) {
		...
		List<Coordinate> path = maze.dfs(start,goal);
		maze.setPath(path);
		System.out.println("\n");
		maze.printMaze();
		maze.clearPath();
