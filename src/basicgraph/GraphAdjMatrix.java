...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		List<Integer> vertTwoHops = new ArrayList<Integer>();
		for (int i = 0; i < getNumVertices(); i++) { // represents the goal column in the two hop matrix
           for (int j=0; j < getNumVertices(); j++) { // index for doing the matrix multiplication
        	   int num2hops = (adjMatrix[v][j])*(adjMatrix[j][i]);
        	   while (num2hops > 0) {
        		   vertTwoHops.add(i);   // every time the matrix multiplication is non-zero, [v][i] is a two-hop possibility.
        	   	   num2hops--;
           	   }
           }
		}
		// for debug only
		//vertTwoHops.sort(null);
		return vertTwoHops;
	}
