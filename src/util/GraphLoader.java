...
public class GraphLoader 
{
	...
	/** File to create a graph connecting all the state capitals of the US for testing TSPSearch.
	 *  All cities are assumed to be fully connected and symmetric. In addition, initializes a Map 
	 *  that relates the City to its GeographicPoint.
	 *  
	 * @param filename - input data file
	 * @param theGraph - the Graph that will be created
	 * @param capitalsMap - Map that relates the City to its GeographicPoint
	 */
	public static void loadCapitals(String filename, roadgraph.MapGraph theGraph,
									HashMap<String,GeographicPoint> capitalsMap)
	{
		String capitalName;
		double lat;
		double lon;
        ...
        try {
            ...
            while ((nextLine = reader.readLine()) != null) {
            	...
            	capitalName = fields[1];
            	lat = Double.parseDouble(fields[2]);
            	lon = Double.parseDouble(fields[3]);
            	GeographicPoint capPoint = new GeographicPoint(lat,lon);
            	theGraph.addVertex(capPoint);
            	capitalsMap.put(capitalName, capPoint);
            }
            reader.close();
        } catch (IOException e) {
        ...
        for (String cap1: capitalsMap.keySet()) {
        	for (String cap2 : capitalsMap.keySet()) {
        		if (!cap1.equals(cap2)) {
        			theGraph.addEdge(capitalsMap.get(cap1), capitalsMap.get(cap2), cap1+" To "+cap2, "motorway", capitalsMap.get(cap1).distance(capitalsMap.get(cap2)));
        		}
        	}
        }
	}
...
//           }
	...
	public static void main(String[] args)
	{
		...
		GraphLoader.createIntersectionsFile("data/maps/losaltos.map", 
					                         "data/intersections/losaltos.intersections");
